(function($) {
            ...
            $.each(data.emails, function(i) {
                ...
                $.each(list, function(j) {
                    ...
                    var li = '<li class="item padding-15" data-email-id="' + id + '"> \
                                <i class="fa fa-comments border-circle unread" style="display:'+unreadclass+'"></i> \
                                ...
                                <div class="inline m-l-15"> \
                                    ...
                                    <p class="body no-margin" style="white-space: pre-line; height: 100%"> \
                                     ' + body + ' \
                                    </p> \
                                </div> \
    ...
    $('body').on('click', '.item', function(e) {
        ...
        var thumbnailWrapper = $(this).find('.thumbnail-wrapper');
         ...
         $(this).find('.unread').css("display","none");
        readmail(id);
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                var ulvotes = $('<ul/>', {
                    "style": "color:#ffffff;list-style: none; padding:0; "
                });
                ...
                var openvotes = email.openvotetasks;


                var closedvotes = 0;
                var closedcomments = 0;


                $.each(openvotes, function(j) {
                    var $this = openvotes[j];
                    var id = $this.id;
                    var appid = $this.appid;
                    var title = $this.title;
                    var checked = $this.completed == true? 'checked' :'';
                    ...
                    var li = '<li><input type="checkbox"'+ checked +' disabled="disabled" style="margin-right: 10px"><a href="/apps/'+appid+'/tasks/'+id+'">'+title+'</a></li>';
                    ulvotes.append(li);
                });

                var opencomments = email.opencommenttasks;

                $.each(opencomments, function(j) {
                    var $this = opencomments[j];
                    var id = $this.id;
                    var appid = $this.appid;
                    var title = $this.title;
                    var checked = $this.completed == true? 'checked' :'';
                ...
                var allopencomments = opencomments.length;
                var allopenvotes = openvotes.length;
                ...
                emailOpened.find('.email-content-body  > .notification-text-body').html(email.body);
                emailOpened.find('.previewvotes').html(ulvotes);
                emailOpened.find('.previewcomments').html(ulcomments);

                emailOpened.find('#completedvotes').text(closedvotes+"/"+allopenvotes);
                emailOpened.find('#completedcomments').text(closedcomments+"/"+allopencomments);
                emailOpened.find('#progresscomments').css("width", (closedcomments/allopencomments)*100.0);
                emailOpened.find('#progressvotes').css("width", (closedvotes/allopenvotes)*100.0);
